...
export async function up({ db, payload }: MigrateUpArgs): Promise<void> {
  ...
  await db.execute(sql.raw(`ALTER TABLE ${aiAssetsTable} ADD COLUMN IF NOT EXISTS "precheck_logs" jsonb`));
}

export async function down({ payload }: MigrateDownArgs): Promise<void> {
  payload.logger.info({
    msg: "down migration for ai_assets.precheck_logs is intentionally a no-op to prevent data loss",
  });
}
